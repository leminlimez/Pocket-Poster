//
//  CachedAsyncGiffy.swift
//  Pocket Poster
//
//  Created by lemin on 7/18/25.
//
...
///
///     CachedAsyncGiffy(url: URL(string: "https://example.com/icon.gif"))
...
///
///     CachedAsyncGiffy(url: URL(string: "https://example.com/icon.gif")) { image in
///         image.resizable()
///     } placeholder: {
///         ProgressView()
///     }
...
///
///     CachedAsyncGiffy(url: URL(string: "https://example.com/icon.gif")) { phase in
...
public struct CachedAsyncGiffy<Content>: View where Content: View {
    
    @State private var phase: AsyncGiffyPhase
    ...
    public init(url: URL?, urlCache: URLCache = .shared,  scale: CGFloat = 1) where Content == Giffy {
    ...
    ///     displays. For example, set a value of `2` for an image that you
    ///     would name with the `@2x` suffix if stored in a file on disk.
    public init(urlRequest: URLRequest?, urlCache: URLCache = .shared,  scale: CGFloat = 1) where Content == Giffy {
    ...
    public init<I, P>(url: URL?, urlCache: URLCache = .shared,  scale: CGFloat = 1, @ViewBuilder content: @escaping (Giffy) -> I, @ViewBuilder placeholder: @escaping () -> P) where Content == _ConditionalContent<I, P>, I : View, P : View {
        let urlRequest = url == nil ? nil : URLRequest(url: url!)
    ...
    ///   - placeholder: A closure that returns the view to show until the
    ///     load operation completes successfully.
    public init<I, P>(urlRequest: URLRequest?, urlCache: URLCache = .shared,  scale: CGFloat = 1, @ViewBuilder content: @escaping (Giffy) -> I, @ViewBuilder placeholder: @escaping () -> P) where Content == _ConditionalContent<I, P>, I : View, P : View {
        self.init(urlRequest: urlRequest, urlCache: urlCache, scale: scale) { phase in
            switch phase {
    ...
    public init(url: URL?, urlCache: URLCache = .shared, scale: CGFloat = 1, transaction: Transaction = Transaction(), @ViewBuilder content: @escaping (AsyncGiffyPhase) -> Content) {
        let urlRequest = url == nil ? nil : URLRequest(url: url!)
    ...
    ///   - content: A closure that takes the load phase as an input, and
    ///     returns the view to display for the specified phase.
    public init(urlRequest: URLRequest?, urlCache: URLCache = .shared, scale: CGFloat = 1, transaction: Transaction = Transaction(), @ViewBuilder content: @escaping (AsyncGiffyPhase) -> Content) {
        ...
        self._phase = State(wrappedValue: .loading)
        do {
            if let urlRequest = urlRequest, let image = try cachedGif(from: urlRequest, cache: urlCache) {
                self._phase = State(wrappedValue: .success(image))
            }
        } catch {
            self._phase = State(wrappedValue: .error)
        }
    }
    ...
    private func load() async {
        do {
            if let urlRequest = urlRequest {
                let (image, metrics) = try await remoteGif(from: urlRequest, session: urlSession)
...
@available(iOS 15.0, macOS 12.0, tvOS 15.0, watchOS 8.0, *)
private extension CachedAsyncGiffy {
    private func remoteGif(from request: URLRequest, session: URLSession) async throws -> (Giffy, URLSessionTaskMetrics) {
        ...
        return (try gif(from: data), metrics)
    }
    
    private func cachedGif(from request: URLRequest, cache: URLCache) throws -> Giffy? {
        guard let cachedResponse = cache.cachedResponse(for: request) else { return nil }
        return try gif(from: cachedResponse.data)
    }
    
    private func gif(from data: Data) throws -> Giffy {
#if os(macOS)
        if let nsImage = NSImage(data: data) {
            return Giffy(nsImage: nsImage)
        } else {
            throw AsyncGiffy<Content>.LoadingError()
        }
#else
        return Giffy(imageData: data)
#endif
    }
}
